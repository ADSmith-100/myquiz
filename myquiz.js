...
let questions = [
  {
    title:
      "What celebration was going on when Fry fell into the cryogenic chamber and was frozen?",
    answers: ["Christmas", "New Year's Eve", "Kwanza", "Halloween"],
    correct: 1,
  },
  {
    title:
      "What did Dr. Zoidberg receive for selling all of his stock in Planet Express?",
...
function showQuestion() {
  ...
  for (let i = 0; i < question.answers.length; i++) {
    $(".quiz form")
      .append(`<p class="answer-line"><input type="radio" id="${i}" name="choice" value="choice">
        <label for="${i}">${question.answers[i]}</label></p>`);
  }
...
function badAnswer() {
  ...
  $(".incorrect span").append(
    ` <span> ${question.answers[smartAnswer]}</span>`
  );
  $(".incorrect span").append("");
  //updateQuestNum();
}
